      ...
      <table class="table table-sm table-striped">
        ...
        <tbody>
          <tr v-for="(data, i) in filterData" :key="i">
            ...
            <td class="text-center">
              <a @click="handleEdit(data)" data-bs-target="#MUserCreate" data-bs-toggle="modal" class="btn btn-sm btn-primary" v-if="can('users-update')">
...
export default {
  ...
  methods: {
    handleEdit(data) {
      ...
      this.$root.$refs.MUserCreate.onLoad();
      let roleid = data.roles.map(function (item) {
        return item.id;
      });
      let branchid = data.mybranch.map(function (item) {
        return item.id;
      });
      this.$root.$refs.MUserCreate.form.fill(data);
      this.$root.$refs.MUserCreate.form.role = roleid;
      this.$root.$refs.MUserCreate.form.branch = branchid;
    },
    handleDelete(data) {
      Swal.fire({
        title: "Are you sure?",
