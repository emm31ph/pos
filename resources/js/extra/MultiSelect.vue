<template>
  <div class="myMultiselect" :style="`z-index:{index}`" @click.prevent="handleClick" @blur="focused = false" tabindex="-1" ref="parent">
    <div class="myMultiselect__selected" v-for="(option, i) in formattedOptions" :key="i" v-show="option.checked">
      {{ option[displayProperty] }}
      <span class="myMultiselect__remove" @click.prevent="handleOptionClick(i)">&times;</span>
    </div>
    <div class="myMultiselect__options" v-show="focused" :style="{ top: optionsTop }" @click.prevent="preventClose">
      <div class="myMultiselect__option" :class="{ 'myMultiselect__option--checked': option.checked }" v-for="(option, i) in formattedOptions" :key="i" @click.prevent="handleOptionClick(i)">
